...
def plot_result(*args):
    ...
    fig, axis = plt.subplots(nrows=1, ncol= number_images, figsize=(12,4))
...
def plot_histogram(image):
    ...
    for index, (ax, color) in enumerate(zip(axis, color_lst)):
        ax.set_litle('{} histogram'.format(color.title()))
        ax.hist(image[:,:,index].ravel(), bons=256, color = color, alpha =0.8)
    fig,tight_layout()
    plt.show()